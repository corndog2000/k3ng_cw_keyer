...
PROGMEM const char hell_font1[] = {B00111111, B11100000, B00011001, B11000000, B01100011, B00000001, B10011100, B00111111, B11100000,    // A
                                   ...
                                   B00111000, B00110000, B11111000, B11000011, B00110011, B00001100, B01111100, B00110000, B01110000
                                  };   // Z
...
PROGMEM const char hell_font2[] = {B00011111, B11100000, B11000000, B11000011, B00000011, B00001100, B00001100, B00011111, B11100000,   // 0
                                   ...
                                   B00000011, B11100011, B00011000, B11000110, B01100011, B00001100, B00001100, B00011111, B11100000
                                  };  // 9
    ...
    if (last_straight_key_state != last_state) {
      ...
      if (element_duration > CW_DECODER_NOISE_FILTER) {                                    // filter out noise
        ...
        if (decode_element_pointer == 16) {
          decode_it_flag = 1;  // if we've filled up the array, go ahead and decode it
        }
      }
  ...
  if (screen_refresh_status == SCREEN_REFRESH_IN_PROGRESS) {
    if (x > lcd_scroll_buffer[y].length()) {
      ...
      if (lcd_scroll_buffer[y].charAt(x) > 0) {
        ...
        tft.backlight();
#endif  // FEATURE_LCD_BACKLIGHT_AUTO_DIM
        tft.print(lcd_scroll_buffer[y].charAt(x));
      }
      x++;
    }
  }
...
#ifndef FEATURE_OLED_SSD1306
          tft.setCursor(((LCD_COLUMNS - lcd_print_string.length()) / 2), row_number);
#else
          if (lcd_print_string.length() <= LCD_COLUMNS )
            tft.setCursor(((LCD_COLUMNS - lcd_print_string.length()) / 2) * 11, ROW_OFFSET * row_number);
          else
            tft.setCursor(0, ROW_OFFSET * row_number);
#endif
...
#ifndef FEATURE_OLED_SSD1306
            tft.setCursor(x, row_number);
#else
            tft.setCursor(x * 11, row_number * ROW_OFFSET);
#endif
            tft.print(" ");
          }
        }
          ...
          while ((keyboard.available()) && (play_memory_prempt == 0)) {
            ...
            if (ps2_keyboard_mode == PS2_KEYBOARD_NORMAL) {
              switch (keystroke) {
                ...
                case PS2_F1_ALT : if (number_of_memories > 0) {
                    repeat_memory_msg(0);
                  } break;
                case PS2_F2_ALT : if (number_of_memories > 1) {
                    repeat_memory_msg(1);
                  } break;
                case PS2_F3_ALT : if (number_of_memories > 2) {
                    repeat_memory_msg(2);
                  } break;
#ifndef OPTION_SAVE_MEMORY_NANOKEYER
                case PS2_F4_ALT : if (number_of_memories > 3) {
                    repeat_memory_msg(3);
                  } break;
                case PS2_F5_ALT : if (number_of_memories > 4) {
                    repeat_memory_msg(4);
                  } break;
                case PS2_F6_ALT : if (number_of_memories > 5) {
                    repeat_memory_msg(5);
                  } break;
                case PS2_F7_ALT : if (number_of_memories > 6) {
                    repeat_memory_msg(6);
                  } break;
                case PS2_F8_ALT : if (number_of_memories > 7) {
                    repeat_memory_msg(7);
                  } break;
                case PS2_F9_ALT : if (number_of_memories > 8) {
                    repeat_memory_msg(8);
                  } break;
                case PS2_F10_ALT : if (number_of_memories > 9) {
                    repeat_memory_msg(9);
                  } break;
                case PS2_F11_ALT : if (number_of_memories > 10) {
                    repeat_memory_msg(10);
                  } break;
                case PS2_F12_ALT : if (number_of_memories > 11) {
                    repeat_memory_msg(11);
                  } break;
#endif //OPTION_SAVE_MEMORY_NANOKEYER
#endif //FEATURE_MEMORIES

                case PS2_DELETE : if (send_buffer_bytes) {
                    send_buffer_bytes--;
                  } break;
        ...
        */
        ...
        void send_char(byte cw_char, byte omit_letterspace)
        {
          ...
          if ((cw_char == 10) || (cw_char == 13)) {
            return;  // don't attempt to send carriage return or line feed
          }
        ...
        void serial_qrss_mode()
        {
          ...
          if (error) {
            ...
            while (primary_serial_port->available() > 0) {
              incoming_serial_byte = primary_serial_port->read();  // clear out buffer
            }
            return;
          } else {
              ...
              if ((send_buffer_array[0] > SERIAL_SEND_BUFFER_SPECIAL_START) && (send_buffer_array[0] < SERIAL_SEND_BUFFER_SPECIAL_END)) {
                ...
                if ((!no_print) && (!cw_send_echo_inhibit)) {
                  ...
                  if (send_buffer_array[0] == 13) {
                    if (primary_serial_port_mode == SERIAL_CLI) {
                      primary_serial_port->write(10); // if we got a carriage return, also send a line feed
                    }
#ifdef FEATURE_COMMAND_LINE_INTERFACE_ON_SECONDARY_PORT
...
#define EEPROM_DUMP_LINES 30
        ...
        void service_paddle_echo()
        {
          ...
          if (paddle_echo_buffer == 111111) {
            paddle_echo_buffer_decode_time = 0;  //this is a special hack to make repeating backspace work
            backspace_flag = 1;
          }
#endif //defined(FEATURE_CW_COMPUTER_KEYBOARD)
        ...
        int serial_get_number_input(byte places, int lower_limit, int upper_limit, PRIMARY_SERIAL_CLS * port_to_use, int raise_error_message) {
          ...
          if (error) {
            ...
            while (port_to_use->available() > 0) {
              incoming_serial_byte = port_to_use->read();  // clear out buffer
            }
            return (-1);
          } else {
            int y = 1;
          ...
          if (set_tx_to > 0) {
            switch (set_tx_to) {
              ...
              case 2: if ((ptt_tx_2) || (tx_key_line_2)) {
                  switch_to_tx_silent(2);
                  port_to_use->print(F("\r\nSwitching to TX #"));
                } port_to_use->println(F("2")); break;
              case 3: if ((ptt_tx_3) || (tx_key_line_3)) {
                  switch_to_tx_silent(3);
                  port_to_use->print(F("\r\nSwitching to TX #"));
                } port_to_use->println(F("3")); break;
              case 4: if ((ptt_tx_4) || (tx_key_line_4)) {
                  switch_to_tx_silent(4);
                  port_to_use->print(F("\r\nSwitching to TX #"));
                } port_to_use->println(F("4")); break;
              case 5: if ((ptt_tx_5) || (tx_key_line_5)) {
                  switch_to_tx_silent(5);
                  port_to_use->print(F("\r\nSwitching to TX #"));
                } port_to_use->println(F("5")); break;
              case 6: if ((ptt_tx_6) || (tx_key_line_6)) {
                  switch_to_tx_silent(6);
                  port_to_use->print(F("\r\nSwitching to TX #"));
                } port_to_use->println(F("6")); break;
            }
          }
        }
#endif
        ...
        */
          ...
          if (serial_number_string.length() < 3 ) {
            if (cut_numbers) {
              if (buffered_sending) {
                ...
                if (keyer_machine_mode != KEYER_COMMAND_MODE) {
                  display_serial_number_character('T'); //Display the SN as well as play it unless playing back after programming for verification(WD9DMP)
                }
                send_char('T', KEYER_NORMAL);
              }
            } else {
              if (buffered_sending) {
                ...
                if (keyer_machine_mode != KEYER_COMMAND_MODE) {
                  display_serial_number_character('0'); //Display the SN as well as play it unless playing back after programming for verification(WD9DMP)
                }
            ...
            if ((serial_number_string[a] == '0') && (cut_numbers)) {
              ...
              if ((serial_number_string[a] == '9') && (cut_numbers)) {
                if (buffered_sending) {
                  ...
                  if (keyer_machine_mode != KEYER_COMMAND_MODE) {
                    display_serial_number_character('N'); //Display the SN as well as play it unless playing back after programming for verification(WD9DMP)
                  }
                  send_char('N', KEYER_NORMAL);
                }
              } else {
                if (buffered_sending) {
                  ...
                  if (keyer_machine_mode != KEYER_COMMAND_MODE) {
                    display_serial_number_character(serial_number_string[a]); //Display the SN as well as play it unless playing back after programming for verification(WD9DMP)
                  }
                        ...
                        if (y < (memory_end(memory_number) + 1)) {
                          ...
                          if ((eeprom_byte_read2 > 48) && (eeprom_byte_read2 < 55)) {
                            switch (eeprom_byte_read2) {
                              ...
                              case 50: if ((ptt_tx_2) || (tx_key_line_2)) {
                                  switch_to_tx_silent(2);
                                } break;
                              case 51: if ((ptt_tx_3) || (tx_key_line_3)) {
                                  switch_to_tx_silent(3);
                                } break;
                              case 52: if ((ptt_tx_4) || (tx_key_line_4)) {
                                  switch_to_tx_silent(4);
                                } break;
                              case 53: if ((ptt_tx_5) || (tx_key_line_5)) {
                                  switch_to_tx_silent(5);
                                } break;
                              case 54: if ((ptt_tx_6) || (tx_key_line_6)) {
                                  switch_to_tx_silent(6);
                                } break;
                            }
                          }
            ...
            if (cwchar > 0) {
              ...
              if (!macro_flag) {
                if (convert_cw_number_to_ascii(cwchar) == '\\') {
                  macro_flag = 1; // if we got the \ macro character, supress spaces
                }
              } else {
        ...
        void initialize_serial_ports() {
          ...
          if (primary_serial_port_mode == SERIAL_CLI) {
            ...
            while (primary_serial_port->available()) {
              primary_serial_port->read(); //clear out buffer at boot
            }
          }
#ifdef DEBUG_MEMORYCHECK
          memorycheck();
          ...
          while (secondary_serial_port->available()) {
            secondary_serial_port->read(); //clear out buffer at boot
          }
#endif
#endif //FEATURE_COMMAND_LINE_INTERFACE_ON_SECONDARY_PORT
        ...
        void initialize_display() {
          ...
          tft.home();
#else
          tft.begin(LCD_COLUMNS, LCD_ROWS);
#endif
#endif

#ifdef FEATURE_LCD_ADAFRUIT_I2C
          tft.setBacklight(lcdcolor);
#endif //FEATURE_LCD_ADAFRUIT_I2C
...
#define USB_KEYBOARD_SPECIAL_MODE_TIMEOUT 5000
          ...
          *((uint8_t*)&modifier) = mod;
          ...
          if ((modifier.bmLeftAlt) || (modifier.bmRightAlt)) {
            switch (key) {
#ifdef FEATURE_MEMORIES
              case 0x3a: if (number_of_memories > 0) {
                  repeat_memory_msg(0);
                } return; break; // F1
              case 0x3b: if (number_of_memories > 1) {
                  repeat_memory_msg(1);
                } return; break;
              case 0x3c: if (number_of_memories > 2) {
                  repeat_memory_msg(2);
                } return; break;
              case 0x3d: if (number_of_memories > 3) {
                  repeat_memory_msg(3);
                } return; break;
              case 0x3e: if (number_of_memories > 4) {
                  repeat_memory_msg(4);
                } return; break;
              case 0x3f: if (number_of_memories > 5) {
                  repeat_memory_msg(5);
                } return; break;
              case 0x40: if (number_of_memories > 6) {
                  repeat_memory_msg(6);
                } return; break;
              case 0x41: if (number_of_memories > 7) {
                  repeat_memory_msg(7);
                } return; break;
              case 0x42: if (number_of_memories > 8) {
                  repeat_memory_msg(8);
                } return; break;
              case 0x43: if (number_of_memories > 9) {
                  repeat_memory_msg(9);
                } return; break;
              case 0x44: if (number_of_memories > 10) {
                  repeat_memory_msg(10);
                } return; break;
              case 0x45: if (number_of_memories > 11) {
                  repeat_memory_msg(11);
                } return; break;
#endif
            } //switch(key)
          ...
          switch (key) {
            ...
            case 0x46: if (send_buffer_bytes) {
                send_buffer_bytes--;
              } return; break;  // DEL
